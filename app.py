...
from wtforms.validators import DataRequired, url as url_validator

from database import Links, engine, Transitions
...
def get_shortened_link(link: str):
    salt = uuid.uuid4().hex.encode()  # If we need a repeatable short link, we need to use a fixed value here
...
def update_short_code(short_code):
    ...
    url = form.url.data
    with Session(engine) as session:
        session.execute(update(Links).where(Links.code == short_code).values(original_link=url))
        session.commit()
    print(request.host_url)
...
def get_statistic(short_code):
    with Session(engine) as session:
        count = session.query(Transitions).filter(
            Transitions.timestamp > datetime.now() - timedelta(days=1)
        ).filter(Transitions.link_code == short_code).count()
